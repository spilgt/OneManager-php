...
class Onedrive {
    ...
    protected function fetch_files_children($path, $page, $getNextlink = false) {
        ...
        if (!$children) {
            ...
            $arr = $this->MSAPI('GET', $url);
            $children_tmp = json_decode($arr['body'], true);
            ...
            for ($page1=$page;$page1>=1;$page1--) {
                ...
                if ($url == '') {
                    if ($page1==1) {
                        ...
                        $arr = $this->MSAPI('GET', $url);
                        $children = json_decode($arr['body'], true);
    ...
    public function Edit($file, $content) {
        ...
        $response=MSAPI('POST',$filename,'{"item": { "@microsoft.graph.conflictBehavior": "replace"  }}');
        $uploadurl=json_decode($response,true)['uploadUrl'];
        echo MSAPI('PUT',$uploadurl,$data);*/
...
<div>
    ...
    protected function MSAPI($method, $path, $data = '')
    {
        ...
        if ($response['stat']==429) {
            ...
            $retryAfter = $res['Retry-After'];
            setConfig(['activeLimit' => $retryAfter], $this->disktag);
        }
        curl_close($ch);
